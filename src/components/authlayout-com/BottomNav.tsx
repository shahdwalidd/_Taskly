...
<nav className="fixed flex   inset-x-0 bottom-0 z-30 bg-surface-low px-7 md:hidden">
{items.map(({icon: Icon,label})=>{
    const isActive = label === "Projects";
    return(
        <button key={label} className={`flex flex-1 items-center flex-col gap-1 py-2 text-xs ${isActive?"text-primary-container" :"text-slate-dark-70"}`}>
<Icon/>
            <span>{label}</span>
        </button>
    )
